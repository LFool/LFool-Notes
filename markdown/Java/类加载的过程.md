...
举个很简单的小例子！现在交给张三一个任务：去找李四

这句话里，「李四」就是一个符号引用，也就是一种字面量，通过这个任务，可以无歧义的完成。但是，张三只知道要去找李四，至于李四住在哪，一概不知！！

当张三真正去执行该任务的时候，会将「李四」这个符号引用转化成直接引用，也就是「李四家的地址」，然后张三就可以根据地址精准找到李四

当`.class`文件还没有加载到内存中的时候，类中的信息均以符号引用存在，可以通过`javap -v Xxx.class`命令查看
