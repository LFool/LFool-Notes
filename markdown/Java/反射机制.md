...
**PS：关于「动态语言」和「动态类型语言」的对比可见 [动态语言 vs 动态类型语言](./动态语言-动态类型语言.html)**
...
加载完类之后，类的类型信息会被存储在方法区中，同时会在堆中生成一个`Class`类型的对象 (**<font color='red'>一个类只有一个 Class 对象</font>**)，这个对象就包含了完整的类的结构信息，**详情可见 [加载阶段](./类加载的过程.html#加载)**
