...
`n`的范围：$0 \le n \le 10^9$，求一个数中 1 的个数最坏需要循环 30 次 ($10^9$ 大概是 $2^{30}$)，所以最坏的时间复杂度为 $O(32 \times n)$
...
private void traversal(char[] s, int i, int path, boolean isLimit) {
    ...
    for (int d = 0; d <= up; d++) {
        ...
        // 如果此时选择上界 2，那么遍历第 1 位的时候也是被限制的；
        // 但是如果此时选择的不是上界 2，那么遍历第 1 位的时候就没有被限制
...
铺垫了这么多，现在由题目 **[数字 1 的个数](https://leetcode.cn/problems/number-of-digit-one/)** 引出今天的主角！！题目很短，意思也很好懂，但也很容易超时。。。
